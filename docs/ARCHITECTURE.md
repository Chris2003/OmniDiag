...
| **Modules** | `src/Modules/` | Diagnostic plugins — 38 granular scanners across endpoint, network, security, identity, and cloud categories. One `.psm1` per scanner. Discovered at runtime. |
...
each one; a full 38-scanner run takes roughly a minute. The engine and the module
